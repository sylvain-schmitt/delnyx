...
class MagicLinkService
{
     ...
     *
     * @param Quote|Amendment|Invoice|CreditNote|Deposit $document
     ...
     */
    private function getEntityType(Quote|Amendment|Invoice|CreditNote|Deposit|Subscription $document): string
    {
        return match (true) {
            ...
            $document instanceof Deposit => 'deposit',
            $document instanceof Subscription => 'subscription',
        };
    }
    ...
    private function getRouteNameForAction(string $entityType, string $action): string
    {
        ...
        $validActions = ['view', 'sign', 'refuse', 'pay', 'apply', 'cancel'];
    ...
    /**
     * Génère un lien de paiement pour un accompte
     */
    public function generateDepositPayLink(
        Deposit $deposit,
        int $expirationDays = self::DEFAULT_EXPIRATION_DAYS
    ): string {
        return $this->generatePublicLink($deposit, 'pay', $expirationDays);
    }

    /**
     * Génère un lien d'annulation d'abonnement
     */
    public function generateSubscriptionCancelLink(
        Subscription $subscription,
        int $expirationDays = self::DEFAULT_EXPIRATION_DAYS
    ): string {
        return $this->generatePublicLink($subscription, 'cancel', $expirationDays);
    }
}
